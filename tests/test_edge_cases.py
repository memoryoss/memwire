...
def test_very_long_content(memory):
    ...
    records = memory.add([{"role": "user", "content": long_text}])
    assert len(records) == 1
    print(f"  Long text ({len(long_text)} chars): {memory.get_stats()['nodes']} nodes")


def test_single_word_input(memory):
    """Single word should work without crashing."""
    records = memory.add([{"role": "user", "content": "hello"}])
    assert len(records) == 1
    result = memory.recall("hello")
    assert result is not None


def test_two_word_input(memory):
    """Two-word input should still work."""
    records = memory.add([{"role": "user", "content": "good morning"}])
    assert len(records) == 1
...
def test_repeated_identical_messages(memory):
    ...
    for _ in range(5):
        memory.add([{"role": "user", "content": "I like chocolate"}])
...
def test_very_similar_messages(memory):
    ...
    result = memory.recall("When is the deadline?")
    all_contents = set()
...
def test_recall_with_empty_query(memory):
    """Empty query string."""
    memory.add([{"role": "user", "content": "Some stored memory"}])
...
def test_rapid_add_recall_cycles(memory):
    ...
    for i in range(10):
        memory.add([{"role": "user", "content": f"Memory number {i} about testing"}])
        result = memory.recall("testing")
    stats = memory.get_stats()
