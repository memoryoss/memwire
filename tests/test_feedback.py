...
def test_feedback_strengthens_aligned_paths(memory):
    ...
    result = memory.recall("Which vendor has organic cotton?")
    edges_before = {
...
def test_feedback_weakens_misaligned_paths(memory):
    ...
    memory.recall("What materials do we use?")
...
def test_feedback_dead_zone(memory):
    ...
    memory.recall("What are we building?")
...
def test_repeated_feedback_accumulates(memory):
    ...
    memory.recall("What is important about orders?")

    # Track weight changes across 5 feedback rounds
    weights_history = []
    for i in range(5):
        memory.recall("What is important about orders?")
        memory.feedback(response="Fast delivery is our top priority for customer satisfaction")
        total_weight = sum(e.weight for e in memory.engine.graph.edges.values())
        weights_history.append(total_weight)

    print(f"  Weight history over 5 rounds: {[f'{w:.2f}' for w in weights_history]}")
    # Weights should generally increase (or stay at max)


def test_feedback_without_recall_is_noop(memory):
    """Feedback without prior recall should do nothing."""
    memory.add([{"role": "user", "content": "Test message for feedback"}])
    stats = memory.feedback(response="Any response")
