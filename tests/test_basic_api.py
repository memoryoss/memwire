...
def test_add_single_message(memory):
    records = memory.add(user_id=TEST_USER, messages=[{"role": "user", "content": "I like pizza"}])
...
def test_add_missing_role_defaults_to_user(memory):
    records = memory.add(user_id=TEST_USER, messages=[{"content": "No role here"}])
    assert records[0].role == "user"


def test_add_missing_content_skipped(memory):
    records = memory.add(user_id=TEST_USER, messages=[{"role": "user"}])
    assert len(records) == 0


def test_recall_returns_result(seeded_memory):
    result = seeded_memory.recall("What materials do we prefer?", user_id=TEST_USER)
...
def test_recall_has_formatted_output(seeded_memory):
    result = seeded_memory.recall("organic materials", user_id=TEST_USER)
    assert isinstance(result.formatted, str)


def test_recall_on_empty_memory(memory):
    result = memory.recall("anything", user_id=TEST_USER)
...
def test_search_returns_results(seeded_memory):
    results = seeded_memory.search("organic", user_id=TEST_USER)
...
def test_search_with_category_filter(seeded_memory):
    results_all = seeded_memory.search("materials", user_id=TEST_USER)
    results_filtered = seeded_memory.search("materials", user_id=TEST_USER, category="nonexistent_category")
...
def test_get_stats(seeded_memory):
    stats = seeded_memory.get_stats(user_id=TEST_USER)
...
def test_close_is_safe(memory):
    memory.add(user_id=TEST_USER, messages=[{"role": "user", "content": "test"}])
